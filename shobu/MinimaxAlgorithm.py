...
def minimax(depth, is_max_player1, game):
    ...
    if depth == 0 or (is_max_player1 and heuristics.objective_test(game.get_boards(), game.get_player1())) != -1:
        heuristics.calc(game.get_boards(), game.get_player1())
        return heuristics.get_value()
    elif depth == 0 or is_max_player1 and (heuristics.objective_test(game.get_boards(), game.get_player2())) != -1:
                ...
                for col in range(COLS):
                    ...
                    if piece != 0 and piece.get_color() == game.get_player1().get_color():
                        ...
                        for move in game.get_player1().get_moves():
                            ...
                            board.change_piece_cell(piece, move)  # movimento passivo efetuado
                                    ...
                                    for row2 in range(ROWS):
                                        for col2 in range(COLS):
                                            ...
                                            if piece2 != 0 and piece2.get_color() == game.get_player1().get_color():
                                                ...
                                                rest = game.get_player1().active_move(board2, vector_move, piece,
                                                                                      color_played)
                                                if rest[0] and len(
                                                        game.get_player1().get_agressive_moves()):  # turn done
                                                    game.get_player1().do_active_move(board2, rest[1], rest[2],
                                                                                      vector_move)

                                                    aux_game = deepcopy(game)

                                                    best = max(best, minimax(depth - 1, False, game))
                                    ...
                                    for row2 in range(ROWS):
                                        for col2 in range(COLS):
                                            ...
                                            if piece2 != 0 and piece2.get_color() == game.get_player2().get_color():
                                                ...
                                                rest = game.get_player2().active_move(board2, vector_move, piece,
                                                                                      color_played)
                                                if rest[0] and len(
                                                        game.get_player2().get_agressive_moves()):  # turn done
                                                    game.get_player2().do_active_move(board2, rest[1], rest[2],
                                                                                      vector_move)

                                                    aux_game = deepcopy(game)

                                                    best = min(best, minimax(depth - 1, True, game))
                                                    game = deepcopy(aux_game)
                                ...
                                for row2 in range(ROWS):
                                    for col2 in range(COLS):
                                        ...
                                        if piece2 != 0 and piece2.get_color() == game.get_player1().get_color():
                                            ...
                                            rest = game.get_player1().active_move(board2, vector_move, piece,
                                                                                  color_played)
                                            if rest[0] and len(game.get_player1().get_agressive_moves()):  # turn done
                                                game.get_player1().do_active_move(board2, rest[1], rest[2],
                                                                                  vector_move)
                                                aux_game = deepcopy(game)
                                    ...
                                    for col2 in range(COLS):
                                        ...
                                        if piece2 != 0 and piece2.get_color() == game.get_player2().get_color():
                                            ...
                                            rest = game.get_player2().active_move(board2, vector_move, piece,
                                                                                  color_played)
                                            if (rest[0] and len(
                                                    game.get_player2().get_agressive_moves()) != 0):  # turn done
                                                game.get_player2().do_active_move(board2, rest[1], rest[2],
                                                                                  vector_move)
                                                aux_game = deepcopy(game)
